...
export function ChargesStep({ rentRollId }: { rentRollId: string }) {
  const { data: serverCharges, isLoading } = useCharges(rentRollId);
  const [charges, setCharges] = useState<ChargeRow[]>([]);
  const update = useUpdateCharges();
  ...
  const handleSave = async () => {
    try {
      await update.mutateAsync({ rentRollId, charges });
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Select the appropriate category for all charge codes, which have been automatically pulled from the rent roll.
      </p>
