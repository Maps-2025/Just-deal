...
export function FloorPlansStep({ dealId, rentRollId }: { dealId: string; rentRollId: string }) {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground mb-4">In this step you can enter details for all floor plans, which have been automatically pulled from the rent roll.</p>
          ...
          <tbody>
            {floorplans.map((fp, i) => (
              <tr key={fp.floor_plan_code} className="border-b hover:bg-muted/30">
                ...
                <td className="px-3 py-1.5"><Select value={fp.unit_type} onValueChange={(v) => handleChange(i, "unit_type", v)}><SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger><SelectContent>{UNIT_TYPES.map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent></Select></td>
                <td className="px-3 py-1.5"><Input type="number" className="h-8 w-16 text-center text-xs" value={fp.bedrooms ?? ""} onChange={(e) => handleChange(i, "bedrooms", parseInt(e.target.value) || null)} /></td>
                <td className="px-3 py-1.5"><Input type="number" className="h-8 w-16 text-center text-xs" value={fp.bathrooms ?? ""} onChange={(e) => handleChange(i, "bathrooms", parseFloat(e.target.value) || null)} /></td>
                ...
                <td className="px-3 py-1.5"><Input className="h-8 text-xs" value={fp.floor_plan_name} onChange={(e) => handleChange(i, "floor_plan_name", e.target.value)} /></td>
              </tr>
            ))}
          </tbody>
        </table>
