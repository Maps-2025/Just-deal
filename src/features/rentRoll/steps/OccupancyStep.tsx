...
export function OccupancyStep({ rentRollId }: { rentRollId: string }) {
  const { data: serverOcc, isLoading } = useOccupancy(rentRollId);
  ...
  const handleSave = async () => {
    try {
      await update.mutateAsync({ rentRollId, occupancy });
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Select the appropriate statuses for all occupancy codes, which have been automatically pulled from the rent roll.
      </p>
