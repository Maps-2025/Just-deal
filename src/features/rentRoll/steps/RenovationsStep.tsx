...
export function RenovationsStep({ dealId, rentRollId }: { dealId: string; rentRollId: string }) {
  ...
  useEffect(() => {
    if (floorplans) setRenovations(floorplans.map((fp) => ({ floor_plan_code: fp.floor_plan_code, renovation_description: "Unrenovated" })));
  }, [floorplans]);

  const handleSave = async () => {
    try { await update.mutateAsync({ rentRollId, renovations }); toast.success("Renovations saved"); }
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground mb-4">Does this rent roll have renovated units?</p>
      <div className="flex gap-3 mb-6">
        <button className={`px-4 py-2 rounded text-sm font-medium border transition-colors ${hasRenovated ? "bg-primary text-primary-foreground border-primary" : "border-border hover:bg-muted"}`} onClick={() => setHasRenovated(true)}>Yes</button>
        <button className={`px-4 py-2 rounded text-sm font-medium border transition-colors ${!hasRenovated ? "bg-primary text-primary-foreground border-primary" : "border-border hover:bg-muted"}`} onClick={() => setHasRenovated(false)}>No</button>
      </div>
      {hasRenovated && (
            ...
            {renovations.map((r, i) => {
              ...
              return (
                <tr key={r.floor_plan_code} className="border-b hover:bg-muted/30">
                  ...
                  <td className="px-3 py-1.5"><Input className="h-8 text-xs" value={r.renovation_description} onChange={(e) => setRenovations((prev) => prev.map((rr, idx) => idx === i ? { ...rr, renovation_description: e.target.value } : rr))} /></td>
                </tr>
              );
            })}
          </tbody>
