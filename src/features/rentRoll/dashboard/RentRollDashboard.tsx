...
export function RentRollDashboard({ rentRollId, onBack }: RentRollDashboardProps) {
  const [activeTab, setActiveTab] = useState("dashboard");
  const { data: dashboard, isLoading } = useRentRollDashboard(rentRollId);
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="border rounded-lg p-5">
                  <h3 className="font-semibold text-sm mb-2">Lease Expiration Schedule</h3>
                  <p className="text-sm text-muted-foreground text-center py-8">No Data Available For Lease Expiration Schedule</p>
                </div>
                <div className="border rounded-lg p-5">
                  <h3 className="font-semibold text-sm mb-2">Leasing Trends</h3>
                  <p className="text-sm text-muted-foreground text-center py-8">No Data Available For Leasing Trends</p>
                </div>
              </div>
            </div>
          ) : null
        )}
        {activeTab === "floorplan" && <FloorPlanSummaryTab rentRollId={rentRollId} />}
        {activeTab === "rentroll" && <RentRollTableTab rentRollId={rentRollId} />}
