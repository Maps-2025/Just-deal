...
export function RentRollTableTab({ rentRollId }: { rentRollId: string }) {
  const { data: units = [], isLoading } = useRentRollUnitsDetailed(rentRollId);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <p className="text-sm text-muted-foreground">{units.length} of {units.length} units</p>
      </div>
        ...
        <table className="w-full text-sm border-collapse">
          <thead className="sticky top-0 z-10 bg-muted/90 backdrop-blur-sm">
            <tr className="border-b">
              ...
              <th className="text-right px-3 py-2 font-medium">Bedrooms</th>
              ...
              <th className="text-right px-3 py-2 font-medium">Contractual Rent</th>
              <th className="text-right px-3 py-2 font-medium">Concessions</th>
            </tr>
          </thead>
          <tbody>
            {units.map((u, i) => (
              <tr key={u.id} className={cn("border-b hover:bg-primary/5", i % 2 === 0 && "bg-muted/20")}>
                ...
                <td className="px-3 py-1.5">
                  <span className={cn(
                    ...
                    u.occupancy_status?.toLowerCase() === "occupied" && "bg-success/10 text-success",
                    u.occupancy_status?.toLowerCase() === "vacant" && "bg-destructive/10 text-destructive"
                  )}>
                    {u.occupancy_status || "—"}
                  </span>
                </td>
                ...
                <td className="px-3 py-1.5 text-right font-mono">{fmt(u.recurring_concessions)}</td>
              </tr>
            ))}
          </tbody>
        </table>
