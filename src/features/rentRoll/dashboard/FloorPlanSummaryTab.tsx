...
export function FloorPlanSummaryTab({ rentRollId }: { rentRollId: string }) {
  const { data: summary = [], isLoading } = useFloorPlanSummary(rentRollId);
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="text-right px-3 py-2 font-medium">Net Area</th>
              ...
              <th className="text-right px-3 py-2 font-medium">Avg Market Rent</th>
              <th className="text-right px-3 py-2 font-medium">Avg Contract Rent</th>
            </tr>
          </thead>
          <tbody>
            {summary.map((fp) => (
