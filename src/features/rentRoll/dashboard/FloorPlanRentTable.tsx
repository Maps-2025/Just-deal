...
interface FloorPlanRentTableProps {
  ...
  monthlyRentMode: "0" | "1" | "2"; // 0=total, 1=per unit, 2=per sf
}

/**
 * Top table: Rent summary with Market Rent (All), Market Rent (Occupied), In-Place Rent, % of Market
 */
export function FloorPlanRentTable({ data, totals, monthlyRentMode }: FloorPlanRentTableProps) {
      ...
      <table className="w-full text-sm border-collapse">
        ...
        <thead>
          <tr className="border-b bg-muted/40">
            <th className="text-center px-4 py-2 font-semibold text-xs uppercase tracking-wider text-muted-foreground border-r" colSpan={2}>
              Unit Information
            </th>
          ...
          <tr className="border-b bg-muted/30">
            <th className="text-left px-4 py-2 font-semibold text-xs text-muted-foreground border-r w-[200px]">Floor Plan</th>
        ...
        <tbody>
          {data.map((row, i) => (
            <tr key={row.floor_plan} className={cn("border-b hover:bg-muted/20 transition-colors", i % 2 === 0 && "bg-background")}>
              <td className="px-4 py-2.5 text-foreground border-r">{row.floor_plan}</td>
        ...
        {totals && (
          <tfoot>
            <tr className="border-t-2 bg-muted/40 font-semibold">
              <td className="px-4 py-2.5 text-foreground border-r">Total / Average</td>
              <td className="border-r w-0" />
