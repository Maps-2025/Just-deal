...
export function RentRollWizard({ rentRollId, onComplete, onCancel }: RentRollWizardProps) {
  const [currentStep, setCurrentStep] = useState(0);
  const finalize = useFinalizeRentRoll();
  ...
  const handleFinish = async () => {
    try {
      await finalize.mutateAsync(rentRollId);
      toast.success("Rent Roll capture complete!");
      onComplete();
    } catch (err: any) {
  ...
  return (
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex-1 overflow-auto p-6">
        {currentStep === 0 && <FloorPlansStep rentRollId={rentRollId} />}
        {currentStep === 1 && <OccupancyStep rentRollId={rentRollId} />}
        {currentStep === 2 && <ChargesStep rentRollId={rentRollId} />}
        {currentStep === 3 && <RenovationsStep rentRollId={rentRollId} />}
        {currentStep === 4 && <AffordabilityStep rentRollId={rentRollId} />}
      </div>
      ...
      <div className="border-t px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {currentStep > 0 && (
            <Button variant="outline" size="sm" onClick={handlePrevious}>Previous</Button>
          )}
          {isLast ? (
            <Button size="sm" onClick={handleFinish} disabled={finalize.isPending}>
