...
export function RentRollModule({ dealId }: RentRollModuleProps) {
  ...
  const deleteRR = useDeleteRentRoll();
  ...
  if (phase === "upload") {
    return <UploadRentRollModal dealId={dealId} onComplete={handleUploadComplete} onCancel={() => setPhase("list")} />;
  }

  if (phase === "mapping" && rentRollId) {
    return <SourceMappingModal rentRollId={rentRollId} headers={uploadHeaders} onComplete={handleMappingComplete} onCancel={() => setPhase("list")} />;
  }

  if (phase === "wizard" && rentRollId) {
    return <RentRollWizard rentRollId={rentRollId} onComplete={handleWizardComplete} onCancel={() => setPhase("list")} />;
  }

  if (phase === "complete" && rentRollId) {
    return (
      <CaptureComplete
        rentRollId={rentRollId}
        onViewDashboard={() => handleViewDashboard()}
        onViewRentRoll={() => handleViewDashboard()}
        onEditRentRoll={() => setPhase("wizard")}
  ...
  if (phase === "dashboard" && activeRentRollId) {
    return <RentRollDashboard rentRollId={activeRentRollId} onBack={() => setPhase("list")} />;
  }
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={() => setPhase("upload")}>
            <Upload className="h-4 w-4 mr-1.5" /> Upload Rent Roll
          </Button>
        </div>
      </div>
          ...
          {rentRolls.map((rr) => (
            <div key={rr.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/30 transition-colors">
              <div className="flex items-center gap-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {rr.total_units ?? 0} units · {rr.occupancy_pct ?? 0}% occupancy
                  </p>
                </div>
                {rr.has_anomalies && (
              ...
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={() => handleViewDashboard(rr.id)}>
                  View
                </Button>
                <Button variant="outline" size="sm" onClick={() => { setRentRollId(rr.id); setPhase("wizard"); }}>
                  <Pencil className="h-3.5 w-3.5 mr-1" /> Edit
                </Button>
                <Button variant="ghost" size="sm" onClick={() => handleDelete(rr.id)} className="text-destructive hover:text-destructive">
